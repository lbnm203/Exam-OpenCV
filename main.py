...
def step_by_step_dbscan(data, epsilon, min_samples):
    st.subheader("Quá trình phân cụm DBSCAN")
...
def run():
    ...
    st.markdown("DBSCAN (Density-Based Spatial Clustering of Applications with Noise) là một thuật toán phân cụm dựa trên mật độ (Density-Based Clustering) được sử dụng phổ biến trong học máy và khai phá dữ liệu. Thuật toán không yêu cầu số lượng cụm phải được định nghĩa trước (như K-Means) và có khả năng xác định các điểm nhiễu (noise).")
    ...
    st.write("Quy trình của DBSCAN")
    st.markdown("1. Chọn một điểm dữ liệu chưa được xử lý.")
    st.markdown("2. Xác định các điểm trong vùng lân cận ε.")
    st.markdown(
        "3. Nếu điểm là điểm lõi, mở rộng cụm bằng cách thêm các điểm lân cận đủ điều kiện (core và border) vào cụm.")
    st.markdown("4. Lặp lại cho tất cả các điểm chưa được xử lý.")
    st.markdown("5. Những điểm không được gán vào cụm nào được coi là nhiễu.")
